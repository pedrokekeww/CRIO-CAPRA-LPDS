document.addEventListener('DOMContentLoaded', () => {
    ...
    const analysisActionsContainer = document.getElementById('analysis-actions-container');
    ...
    function stopCameraAndInference() {
        ...
        if (cameraResultPreview) {
            cameraResultPreview.classList.add('hidden');
            cameraResultPreview.src = '';
        }
    }
    ...
    modeFileBtn.addEventListener('click', () => {
        ...
        if (analysisActionsContainer) analysisActionsContainer.style.display = 'flex';
    ...
    modeCameraBtn.addEventListener('click', async () => {
        const modelId = activeModelSelect.value;
        if (!modelId) {
            alert('Por favor, selecione um modelo de I.A. primeiro antes de usar a câmera.');
            return;
        }
        ...
        if (analysisActionsContainer) analysisActionsContainer.style.display = 'none';
        
        try {
            stream = await navigator.mediaDevices.getUserMedia({ video: true });
            cameraVideo.srcObject = stream;
            
            await new Promise(resolve => cameraVideo.onplaying = resolve);

            const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
            ws = new WebSocket(`${protocol}//${window.location.host}/ws/analyze/${modelId}`);
            
            ws.onmessage = (event) => {
                cameraResultPreview.src = event.data;
                cameraResultPreview.classList.remove('hidden');
            };
            ...
            ws.onopen = () => {
                inferenceInterval = setInterval(() => {
                    if (ws.readyState === WebSocket.OPEN && cameraVideo.videoWidth) {
                        const ctx = cameraCanvas.getContext('2d');
                        cameraCanvas.width = cameraVideo.videoWidth;
                        cameraCanvas.height = cameraVideo.videoHeight;
                        ctx.drawImage(cameraVideo, 0, 0, cameraCanvas.width, cameraCanvas.height);
                        
                        const base64Frame = cameraCanvas.toDataURL('image/jpeg', 0.7);
                        ws.send(base64Frame);
                    }
                }, 200); // 5 FPS
            };
            ...
            console.error('Camera/WS access error:', err);
            alert('Erro ao acessar a câmera ou conectar ao servidor em tempo real.');
        }
    });

    activeModelSelect.addEventListener('change', () => {
        if (cameraArea.style.display === 'flex') {
            stopCameraAndInference();
            modeCameraBtn.click();
        }
    });
    ...
    captureBtn.addEventListener('click', () => {
        ...
        cameraCanvas.toBlob((blob) => {
            ...
            // Preenche um nome automaticamente se estiver vazio
            const analysisNameInput = document.getElementById('analysis-name');
            if (!analysisNameInput.value.trim()) {
                analysisNameInput.value = `Captura ${new Date().toLocaleTimeString()}`;
            }
            
            // Executa a análise automaticamente para gerar e salvar os metadados reais
            setTimeout(() => {
                runAnalysis();
            }, 300);
